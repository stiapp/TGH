...
public class Idos2 {
    ...
    private static void vypisSpojeni(int kamJedu, Vrchol zdrojovyVrchol) {
        ...
        if (cilovy_uzel == null || cilovy_uzel.getVzdalenost() == Integer.MAX_VALUE) {
            ...
            Vrchol pridavany_vrchol = null;

            for (int x = zdrojovyVrchol.getCisloStanice(); x < cilovy_uzel.getCisloStanice(); x++) {
                int maximum_casu_x_te_stanice_v_ceste = Integer.MIN_VALUE;
                for (Vrchol node : cilovy_uzel.getNejkratsiCesta()) {
                    if (node.getCisloStanice() == x && maximum_casu_x_te_stanice_v_ceste < node.getCasVeStanici()) {
                        maximum_casu_x_te_stanice_v_ceste = node.getCasVeStanici();
                        pridavany_vrchol = node;
                    }
                }
                //mam vrchol v case pred jeho opustenim k dalsi stanici
                if (pridavany_vrchol != null) {
                    vysledna_cesta.add(pridavany_vrchol);
                }
                pridavany_vrchol = null;
            }
